...
class Services_WTF_Test {
    ...
    private $username = "";
    private $password = "";
    private $user_agent = "Services_WTF_Test_php/0.2 (+http://gtmetrix.com/api/)";
    protected $testid = "";
    ...
    protected function checkid() {
        if ( empty( $this->testid ) ) {
            $this->error = "No testid! Please start a new test or load an existing test first.";
            return false;
        }
     ...
     *
     * returns the testid on success, false otherwise;
     */
     ...
     *
     * $testid  string  The existing test's test ID
     *
     * testid must be valid, or else all query methods will fail
     */
     ...
     *
     * Precondition: member testid is not empty
     *
     ...
     * get_testid()
     *
     * Returns the testid, false if testid is not set
     */
    public function get_testid() {
        return ($this->testid) ? $this->testid : false;
    }
     ...
     *
     * returns true on successful poll, or false on network error or no testid
     */
     ...
     */
    public function download_resources( $items = null, $location = './' ) {
        if ( !$this->completed() )
            return false;
        ...
        foreach ( $items as $item ) {
            ...
            $file = fopen( $location . $item . '.' . $resource_types[$item], "w" );
